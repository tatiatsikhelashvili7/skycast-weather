...
import {
  ...
  openMeteoForecast,
  openMeteoByCoords,
  openMeteoForecastByCoords,
  geocodeCity,
  reverseGeocode,
} from "./openMeteo";

/**
 * Weather service with a tiered data strategy:
 *
 * 1. Georgian cities  → amindi.ge (most accurate local data, 3-min TTL)
 * 2. Everything else  → Open-Meteo (free worldwide, no key, 10-min TTL)
 * 3. Fallback         → OpenWeatherMap (only if OPENWEATHER_API_KEY is set)
 *
 * Open-Meteo aggregates from official national weather services (NOAA,
 * DWD, ECMWF, JMA, etc.) and is refreshed hourly at the source.
 */
...
export async function getCurrentWeather(city: string): Promise<any> {
  // 1. Georgian cities → amindi.ge
  if (isGeorgianCity(city)) {
    const amindi = await amindiGetCurrent(city);
    if (amindi) return amindi;
  }

  // 2. Worldwide → Open-Meteo (always available, no key needed)
  const om = await openMeteoCurrent(city);
  if (om) return om;

  // 3. Fallback → OpenWeatherMap (if key is set)
...
/**
 * Forward geocoding — turn a free-text city query into coordinates.
 * Throws `"City not found"` if the query couldn't be resolved.
 */
export async function geocodeQuery(query: string) {
  const geo = await geocodeCity(query);
  if (!geo) throw new Error(`We couldn't find "${query}". Try another spelling?`);
  return geo;
}

/** Reverse geocoding — (lat, lon) → nearest populated place. Never throws. */
export async function reverseGeocodeCoords(lat: number, lon: number) {
  return reverseGeocode(lat, lon);
}

/**
 * One-shot "bundle" — fetches both `current` and `forecast` in a single
 * coordinate-based round-trip. Used by the new client search pipeline
 * so we don't hit the upstream geocoder twice and can share cached
 * Open-Meteo responses between the two payloads.
 */
export async function getBundleByCoords(
  lat: number,
  lon: number
): Promise<{ current: any; forecast: any }> {
  const [current, forecast] = await Promise.all([
    openMeteoByCoords(lat, lon),
    openMeteoForecastByCoords(lat, lon),
  ]);

  if (current && forecast) return { current, forecast };
  ...
  throw new Error(
    `Weather services are unreachable for (${lat.toFixed(3)}, ${lon.toFixed(3)})`
  );
}

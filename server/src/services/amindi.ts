/**
 * amindi.ge scraper — parses the Georgian weather site HTML and returns
 * data in the OpenWeatherMap shape the frontend already expects.
 *
 * amindi.ge does NOT publish an API, but the city pages render fully on
 * the server with a predictable structure, so we fetch + regex-extract.
 *
 * Updates every 3 minutes (short TTL since the user wants "always fresh").
 */

import { db } from "../db";

const BASE = "https://amindi.ge/ka/city";
const AMINDI_TTL_MS = 3 * 60 * 1000; // 3 minutes — always fresh

/* ─────────── City mapping (Georgian + English → amindi slug) ─────────── */
...
function getCached(key: string): any | null {
  ...
  if (!row) return null;
  if (Date.now() - row.fetched_at > AMINDI_TTL_MS) return null;
  try {
...
async function fetchHtml(slug: string): Promise<string> {
  const res = await fetch(`${BASE}/${slug}`, {
    headers: {
      "User-Agent":
        "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 SkyCastBot/1.0",
      "Accept-Language": "ka,en;q=0.8",
    },
  });
  if (!res.ok) throw new Error(`amindi.ge ${res.status} for ${slug}`);
  return res.text();
}
...
// amindi uses weather_small_XX.png and XX.png (01-30) — rough mapping to
// OpenWeather-ish main conditions + icon codes so the UI themes still work.
const ICON_MAP: Record<string, { main: string; description: string; icon: string; id: number }> = {
...
function iconFromAmindi(imgPath: string | null): {
  ...
  if (!imgPath) return { main: "Clear", description: "clear sky", icon: "01d", id: 800 };
  const m = /weather(?:_small)?_(\d{2})\.png/i.exec(imgPath) || /\/(\d{2})\.png/.exec(imgPath);
  const code = m ? m[1] : "01";
  return (
    ICON_MAP[code] ?? { main: "Clouds", description: "cloudy", icon: "03d", id: 802 }
  );
}

/* ─────────── Georgian → English phrase hints (purely for description text) ─────────── */
...
function translatePhrase(ka: string): string {
  for (const key of Object.keys(PHRASE_HINTS)) {
    if (ka.includes(key)) return PHRASE_HINTS[key];
  }
  return ka;
}
...
interface ParsedAmindi {
  ...
  humidity: number;
  visibilityKm: number;
  hours: { hour: string; temp: number; icon: ReturnType<typeof iconFromAmindi> }[];
  days: {
    weekday: string;
    date: string;
    tempMin: number;
...
function parseAmindiHtml(html: string): ParsedAmindi {
  ...
  const phraseKa =
    pick(/<div class="current-phrase">([^<]+)<\/div>/, html)?.trim() ?? "";

  const tempMatch = /<div class="degrees">\s*<img src="([^"]+)"[^>]*\/>\s*<span><span[^>]*>([-\d.]+)<\/span>/.exec(
    html
  );
  ...
  // Wind block
  const windBlock = pick(
    /<div class="wind">[\s\S]*?<div class="wind-text">([\s\S]*?)<\/div>\s*<\/div>/,
    html
  );
  let windKph = 0;
  let windDir = "";
  ...
  // More-info right column — grab it straight from the full HTML
  const rightBlock = pick(
    /<div class="more-info-right">([\s\S]*?)<\/div>\s*<\/div>/,
    html
  );
  let feelsLike = temp;
  let humidity = 0;
  let visibilityKm = 10;
  if (rightBlock) {
    const pVals: string[] = pickAll(/<p>([\s\S]*?)<\/p>/g, rightBlock).map((m) =>
      m[1].replace(/<[^>]*>/g, "").replace(/\s+/g, " ").trim()
    );
    if (pVals[0]) feelsLike = toNum(pVals[0]);
    for (const v of pVals) {
      if (v.includes("%")) humidity = toNum(v);
      else if (/\bkm\b/.test(v) && !/kph/i.test(v)) visibilityKm = toNum(v);
    }
  }

  // Hourly
  const hoursBlock = pick(
    /<div class="weather-hours[^"]*">([\s\S]*?)<a href="\/ka\/hourly\//,
    html
  );
  const hours: ParsedAmindi["hours"] = [];
  if (hoursBlock) {
    // Pattern: <div class="col-5 ...">N საათი</div> <div class="col-2 ..."><img src="..."/></div> <div class="col-5 ..."><span unit="C">T</span>°</div>
  ...
  // Days
  const daysBlock = pick(
    /<div class="weather-days">[\s\S]*?<div class="weather-days-right">([\s\S]*?)<\/div>\s*<\/div>/,
    html
  );
...
/* ─────────── Shape into OpenWeatherMap-compatible responses ─────────── */

function toOWMCurrent(cityInput: string, p: ParsedAmindi): any {
  const descriptionKa = p.phraseKa || p.icon.description;
  const description = translatePhrase(descriptionKa);
  const now = Math.floor(Date.now() / 1000);

  return {
    coord: { lat: 41.72, lon: 44.78 }, // Georgia-ish; UI doesn't rely on precision
    weather: [
      {
        id: p.icon.id,
        main: p.icon.main,
    ...
    main: {
      ...
      temp_min: p.temp - 2,
      temp_max: p.temp + 2,
      pressure: Math.round((p.pressure || 101.3) * 10), // kpa → hPa
      humidity: p.humidity,
    },
    visibility: Math.round((p.visibilityKm || 10) * 1000),
    wind: {
      speed: Math.round((p.windKph / 3.6) * 10) / 10, // kph → m/s (OWM convention)
...
function toOWMForecast(cityInput: string, p: ParsedAmindi): any {
  ...
  // Build hourly forecast from amindi hours (3h-ish buckets; we have 1h entries)
  ...
  // Daily samples (mid-day representative point for each of the next 7 days).
  // We emit these even when hourly is present so the 7-day chart always has
  // coverage — amindi's hourly feed is only ~16h long on its own.
  ...
  // Deduplicate days already covered by the hourly window so aggregation
  // doesn't get noisy when amindi's hourly span overlaps tomorrow.
  const hourlyDayKeys = new Set(
    hourlyList.map((h) => h.dt_txt.slice(0, 10))
  );
  const supplementalDaily = dailyList.filter(
    (d) => !hourlyDayKeys.has(d.dt_txt.slice(0, 10))
  );
...
export async function amindiGetCurrent(city: string): Promise<any | null> {
  ...
  try {
    ...
    setCached(key, out);
    // Also cache forecast from the same HTML to avoid refetch
    setCached(`amindi:forecast:${slug}`, toOWMForecast(city, parsed));
    return { ...out, cached: false };
  } catch (err) {
    console.error("[amindi] fetch failed:", (err as Error).message);
    return null;
  }
}
...
export async function amindiGetForecast(city: string): Promise<any | null> {
  ...
  const key = `amindi:forecast:${slug}`;
  const cached = getCached(key);
  if (cached) return { ...cached, cached: true };
  ...
  try {
    ...
    setCached(key, out);
    setCached(`amindi:current:${slug}`, toOWMCurrent(city, parsed));
    return { ...out, cached: false };
  } catch (err) {
    console.error("[amindi] fetch failed:", (err as Error).message);
    return null;
...
export function amindiSupportedCities(): string[] {
  // Unique Georgian names (keys that aren't pure-latin duplicates)

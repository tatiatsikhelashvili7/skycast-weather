...
const BDC_REVERSE = "https://api.bigdatacloud.net/data/reverse-geocode-client";
...
export async function geocodeCity(city: string): Promise<GeocodeResult | null> {
    ...
    // Open-Meteo geocoding ზოგჯერ უკეთ მუშაობს, როცა ვუთითებთ `language`-ს
    // (მაგ: `東京`/`თბილისი`). ამიტომ ვაკეთებთ მცირერიცხოვან fallback-ებს.
    const hasGeorgian = /[\u10A0-\u10FF]/.test(city);
    const hasJapanese = /[\u3040-\u30FF\u3400-\u4DBF\u4E00-\u9FFF]/.test(city);

    const urls: string[] = [];
    if (hasGeorgian)
        urls.push(`${GEOCODE}?name=${encoded}&count=5&language=ka&format=json`);
    if (hasJapanese)
        urls.push(`${GEOCODE}?name=${encoded}&count=5&language=ja&format=json`);

    // Generic fallback: English (broad coverage).
    urls.push(`${GEOCODE}?name=${encoded}&count=5&language=en&format=json`);
    // Last fallback: without explicit language.
    urls.push(`${GEOCODE}?name=${encoded}&count=5&format=json`);
...
async function fetchForecast(lat: number, lon: number, timezone?: string): Promise<any> {
    ...
    const params = new URLSearchParams({
        ...
        timezone: tz,
        current: "temperature_2m,apparent_temperature,relative_humidity_2m,weather_code,wind_speed_10m,wind_direction_10m,pressure_msl,is_day,cloud_cover,visibility",
        hourly: "temperature_2m,apparent_temperature,weather_code,relative_humidity_2m,wind_speed_10m,wind_direction_10m,pressure_msl,cloud_cover,visibility,is_day",
...
function toUnixSecondsFromLocalIso(localIso: string | undefined, utcOffsetSeconds: unknown): number {
    if (!localIso)
        return 0;
    const offset = typeof utcOffsetSeconds === "number" && Number.isFinite(utcOffsetSeconds)
        ? utcOffsetSeconds
        : 0;
    const msAssumingUtc = new Date(localIso + "Z").getTime();
    if (!Number.isFinite(msAssumingUtc))
        return 0;
    return Math.floor((msAssumingUtc - offset * 1000) / 1000);
}

function buildCurrent(city: GeocodeResult, raw: any): any {
    const c = raw.current;
    const isDay = Boolean(c.is_day);
    const cond = wmoToCondition(c.weather_code, isDay);
    const nowSec = toUnixSecondsFromLocalIso(c.time, raw?.utc_offset_seconds);
    const daily = raw.daily;
    const sunrise = toUnixSecondsFromLocalIso(daily?.sunrise?.[0], raw?.utc_offset_seconds);
    const sunset = toUnixSecondsFromLocalIso(daily?.sunset?.[0], raw?.utc_offset_seconds);
    return {
        ...
        sys: {
            ...
            sunrise,
            sunset,
            timezone: raw?.timezone ?? city.timezone ?? undefined,
            utc_offset_seconds: typeof raw?.utc_offset_seconds === "number" ? raw.utc_offset_seconds : undefined,
        },
        name: city.name,
...
function buildForecast(city: GeocodeResult, raw: any): any {
    ...
    const hoursBack = hourly.time.findIndex((t: string) => {
        return toUnixSecondsFromLocalIso(t, raw?.utc_offset_seconds) * 1000 >= Date.now();
    });
    const startIdx = hoursBack >= 0 ? hoursBack : 0;
    const list = [];
    for (let i = startIdx; i < hourly.time.length && list.length < 40; i += 3) {
        const t = hourly.time[i];
        const dt = toUnixSecondsFromLocalIso(t, raw?.utc_offset_seconds);
        const isDay = typeof hourly.is_day?.[i] === "number" ? hourly.is_day[i] === 1 : true;
        ...
        list.push({
            ...
            main: {
                ...
                feels_like: hourly.apparent_temperature?.[i] ?? hourly.temperature_2m[i],
            ...
            wind: {
                ...
                deg: Math.round(hourly.wind_direction_10m?.[i] ?? 0),
            },
            clouds: { all: Math.round(hourly.cloud_cover?.[i] ?? 0) },
            visibility: Math.round(hourly.visibility?.[i] ?? 10000),
...
export async function openMeteoForecastByCoords(lat: number, lon: number): Promise<any | null> {

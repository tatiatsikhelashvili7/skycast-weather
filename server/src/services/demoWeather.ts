...
export function mockCurrent(cityQuery: string): any {
    ...
    const hourSeed = (Date.now() / 3600000) % 1;
...
export function mockForecast(cityQuery: string): any {
    ...
    const list = Array.from({ length: 40 }).map((_, i) => {
        ...
        const dtTxt = `${d.getUTCFullYear()}-${String(d.getUTCMonth() + 1).padStart(2, "0")}-${String(d.getUTCDate()).padStart(2, "0")} ${String(d.getUTCHours()).padStart(2, "0")}:00:00`;
        return {
            dt: ts,
            main: {
                temp: Number(temp.toFixed(1)),

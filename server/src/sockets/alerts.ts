...
/**
 * Rotating "Live Facts" — global weather observations, pushed to every
 * connected client as a low-severity heartbeat. Gives the UI a genuine
 * live feel even when conditions are calm.
 */
const liveFacts = [
...
export function attachAlertStream(io: Server): void {
  io.on("connection", (socket) => {
    ...
    // Warm-welcome fact on connect (so the UI has something to show immediately)
    ...
    socket.on("alerts:subscribe", (payload: { city?: string; condition?: string } = {}) => {
  ...
  // Randomized severe-style alert every 60–120s
  const scheduleAlert = () => {
  ...
  // Live fact every 45–75s — faster cadence, friendlier tone
  const scheduleFact = () => {

...
interface Props {
  code: string;
  members: number;
  /** Called when the user triggers a copy; receives the shareable URL. */
 ...
 *
 * The tilt stays gentle (±10deg max) so it feels premium, never gimmicky.
 */
export function RoomCodeCard({ code, members, onCopy }: Props) {
  ...
  // Spring-smoothed rotations for that buttery iOS feel
  ...
  return (
    <div style={{ perspective: 1200 }} className="w-full">
      ...
      >
        ...
        {/* Indigo/cyan ambient corner glows */}
        <div
          aria-hidden
          className="pointer-events-none absolute -top-16 -left-16 w-72 h-72 rounded-full"
          style={{
            background:
              "radial-gradient(circle, rgba(99,102,241,0.28), transparent 65%)",
            filter: "blur(40px)",
          }}
        />
        <div
          aria-hidden
          className="pointer-events-none absolute -bottom-16 -right-10 w-72 h-72 rounded-full"
          style={{
            background:
        ...
        {/* The code — each letter lifts slightly out of the card */}
        <div
          className="relative mt-5 flex items-center justify-center gap-1.5 sm:gap-2 md:gap-3"
        ...
        >
          {letters.map((ch, i) => (
            <motion.div
              ...
              className="w-12 h-14 sm:w-14 sm:h-16 md:w-16 md:h-20 rounded-2xl flex items-center justify-center text-2xl sm:text-3xl md:text-4xl font-light tracking-widest"
              style={{
                background:
                  "linear-gradient(180deg, rgba(255,255,255,0.08), rgba(255,255,255,0.02))",
                border: "1px solid rgba(255,255,255,0.12)",
                boxShadow:
                  "0 8px 24px -8px rgba(2,6,23,0.7), inset 0 1px 0 rgba(255,255,255,0.1)",
              }}
            >
              <span
        ...
        <p
          className="relative text-center text-xs text-white/55 mt-4"
        ...
        >
          <motion.button
            ...
            whileTap={{ scale: 0.96 }}
            onClick={handleCopy}
            className="relative inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl text-sm font-medium text-white border border-white/15 overflow-hidden"
            style={{
              background:
                "linear-gradient(135deg, rgba(99,102,241,0.85), rgba(34,211,238,0.8))",
              boxShadow:
                "0 10px 30px -10px rgba(99,102,241,0.55), inset 0 1px 0 rgba(255,255,255,0.2)",
            }}
          >
            {/* Ripple burst */}
            <AnimatePresence>
        ...
        {/* Small helper row with alternative share */}
        <div
          className="relative mt-3 flex justify-center text-[11px] text-white/45"
          style={{ transform: "translateZ(20px)" }}
        >
